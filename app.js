...
function startDataListeners(){
  db.collection('categories').where('type','==','expense')
    .onSnapshot(snap=>{
      state.categoriesExpense = snap.docs.map(d=>({id:d.id, ...d.data()}))
        .sort((a,b)=>a.name.localeCompare(b.name));
      renderAll();
    }, err=>{ console.error('categorias gasto:', err); showToast('Erro a carregar categorias de gastos.'); });

  db.collection('categories').where('type','==','saving')
    .onSnapshot(snap=>{
      state.categoriesSaving = snap.docs.map(d=>({id:d.id, ...d.data()}))
        .sort((a,b)=>a.name.localeCompare(b.name));
      renderAll();
    }, err=>{ console.error('categorias poupança:', err); showToast('Erro a carregar categorias de poupança.'); });
  ...
  db.collection('expenses')
    .onSnapshot(snap=>{
      ...
      renderAll();
    }, err=>{ console.error('gastos:', err); showToast('Erro a carregar gastos.'); });
  ...
  db.collection('savings')
    .onSnapshot(snap=>{
      state.savings = snap.docs.map(d=>({id:d.id, ...d.data()}))
        .sort((a,b)=> b.date.localeCompare(a.date));
      renderAll();
    }, err=>{ console.error('poupanças:', err); showToast('Erro a carregar poupanças.'); });
}
